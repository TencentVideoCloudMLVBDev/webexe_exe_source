...
DialogMessage::ResultCode DialogMessage::exec(const QString& title, int btns, QWidget *parent)
{
    DialogMessage msgBox(parent);
...
DialogMessage::ResultCode DialogMessage::exec(const QString& title, int btns, int delayCloseMs, QWidget *parent)
{
    DialogMessage msgBox(parent);
    msgBox.ui.label_title->setText(title);
    msgBox.ui.btn_ok->setVisible(DialogMessage::OK == (DialogMessage::OK & btns));
...
void DialogMessage::closeEvent(QCloseEvent *event)
{
    ...
    this->done(DialogMessage::Rejected);
}

void DialogMessage::timerEvent(QTimerEvent *event)
{
    this->done(DialogMessage::Timeout);
}
...
{
    this->done(DialogMessage::Accepted);
}
