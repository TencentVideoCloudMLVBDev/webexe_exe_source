...
LiveShareVideo::LiveShareVideo(QWidget *parent)
    : QDialog(parent)
    , m_pressed(false)
    , m_point()
{
...
void LiveShareVideo::setUserInfo(std::string userName, std::string userID, bool bAdd, bool bStart)
{
	...
	if (bAdd)
	{
		...
		if (!idle)
		{
			...
			cameraWidget->setCancelLinkMic(m_cancelLinkMic);
...
void LiveShareVideo::setWidth(int width)
{
	m_width = width;
...
void LiveShareVideo::setCameraSize(int cameraSize)
{
	m_cameraSize = cameraSize;
	LINFO(L"setCameraSize :%d",cameraSize);
	initCameraWidget();
	setWidth(m_width);
}

void LiveShareVideo::setCancelLinkMic(bool supportCancelLinkMic)
{
	m_cancelLinkMic = supportCancelLinkMic;
}

void LiveShareVideo::mousePressEvent(QMouseEvent *event)
{
...
void LiveShareVideo::mouseMoveEvent(QMouseEvent *event)
{
    if (m_pressed)
    {
...
void LiveShareVideo::mouseReleaseEvent(QMouseEvent *event)
{
    Q_UNUSED(event);
...
void LiveShareVideo::on_btn_max_clicked()
{
	if (m_bMax)
	{
		m_bMax = false;
...
void LiveShareVideo::initCameraWidget()
{
...
void LiveShareVideo::on_btn_close_clicked()
{
	done(0);
}